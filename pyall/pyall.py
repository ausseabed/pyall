...
class I_INSTALLATION:
    def __init__(self, fileptr, numberOfBytes):
        # assign the KM code for this datagram type
        self.typeOfDatagram = "I"
...
class R_RUNTIME:
    ...
    def read(self):
        ...
        if (not isBitSet(self.filterIdentifier, 4)) & (
            not isBitSet(self.filterIdentifier, 7)
        ):
            self.filterSetting += "+RangeGatesNormal"
        if (isBitSet(self.filterIdentifier, 4)) & (
            not isBitSet(self.filterIdentifier, 7)
        ):
            self.filterSetting += "+RangeGatesLarge"
        if (not isBitSet(self.filterIdentifier, 4)) & (
            isBitSet(self.filterIdentifier, 7)
        ):
